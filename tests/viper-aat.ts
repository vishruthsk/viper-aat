import { ViperAAT } from '../src/viper-aat'
...
describe('ViperAAT Class tests', () => {
    describe('Valid AAT parameters', () => {
        it('should create a valid ViperAAT instance', async () => {
            const viperAAT = await ViperAAT.from(version, clientPublicKey, applicationPublicKey, applicationPrivateKey)
            expect(viperAAT).to.not.be.an.instanceof(TypeError)
            expect(viperAAT).to.be.an.instanceof(ViperAAT)
            expect(viperAAT.isValid()).to.equal(true)
        })

        it('should create a valid ViperAAT instance with an already created signature', () => {
            const viperAAT = new ViperAAT(version, clientPublicKey, applicationPublicKey, applicationSignature)
            expect(viperAAT).to.not.be.an.instanceof(TypeError)
            expect(viperAAT).to.be.an.instanceof(ViperAAT)
            expect(viperAAT.isValid()).to.equal(true)
        })
    ...
    describe('Invalid Hex AAT parameters', () => {
        it('should fail given an invalid hex clientPublicKey', async () => {
            try {
                await ViperAAT.from(version, invalidHexClientPublicKey, applicationPublicKey, applicationPrivateKey)
            } catch (error) {
        ...
        it('should fail given an invalid hex applicationPublicKey', async () => {
            try {
                await ViperAAT.from(version, clientPublicKey, invalidHexApplicationPublicKey, applicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(TypeError)
            }
        ...
        it('should fail given an invalid hex applicationPrivateKey', async () => {
            try {
                await ViperAAT.from(version, clientPublicKey, applicationPublicKey, invalidHexApplicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(TypeError)
            }
        })
        ...
        it('Should fail given an invalid hex Application Signature', () => {
            try {
                new ViperAAT(version, clientPublicKey, applicationPublicKey, invalidHexApplicationSignature)
            } catch (error) {
    ...
    describe('Unsupported or invalid AAT Scenarios', () => {
        it('should fail given an unsupported version', async () => {
            try {
                await ViperAAT.from("1.0.2", clientPublicKey, applicationPublicKey, applicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        it('should fail given an Application Signature that isnt related to the Application Public Key', () => {
            try {
                new ViperAAT(version, clientPublicKey, applicationPublicKey, invalidApplicationSignature)
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
    ...
    describe('Wrong AAT parameters', () => {
        it('should fail given an empty version', async () => {
            try {
                await ViperAAT.from("", clientPublicKey, applicationPublicKey, applicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        ...
        it('should fail given an empty clientPublicKey', async () => {
            try {
                await ViperAAT.from(version, "", applicationPublicKey, applicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        ...
        it('should fail given an empty applicationPublicKey', async () => {
            try {
                await ViperAAT.from(version, clientPublicKey, "", applicationPrivateKey)
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
        ...
        it('should fail given an empty applicationPrivateKey', async () => {
            try {
                await ViperAAT.from(version, clientPublicKey, applicationPublicKey, "")
            } catch (error) {
                expect(error).to.be.an.instanceof(Error)
            }
        })
